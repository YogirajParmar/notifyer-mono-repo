  ...
  private async ensureLogDirectory(): Promise<void> {
    try {
      ...
      if (error.code === "ENOENT") {
        ...
        this.log(
          "error",
          `Failed to access log directory: ${error instanceof Error ? error.message : String(error)}`
        );
      }
    }
  }
    ...
    try {
      ...
      const deletePromises = files.map(async (file) => {
        ...
        try {
          ...
          this.log(
            "error",
            `Error handling file ${file}: ${err instanceof Error ? err.message : String(err)}`
          );
        }
      });
      ...
      this.log(
        "error",
        `Error reading log directory: ${err instanceof Error ? err.message : String(err)}`
      );
    }
  }
